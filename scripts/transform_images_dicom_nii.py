...
def main():
    ...
    parser.add_argument(
        "-o",
        "--orig_data_base_dir",
        default="data/manifest-1600709154662/LIDC-IDRI/",
    ...
    parser.add_argument(
        ...
        "--data_base_dir",
        default="data/images/",
        help="directory to store transformed images",
    )
    parser.add_argument(
        "-i",
        "--data_list_file_path",
        default="data/calibration_images.json",
        help="list of calibration images to transform",
    )
    parser.add_argument(
        "-m",
        "--dicom_meta_data_csv",
        default="data/manifest-1600709154662/metadata.csv",
        help="location of LIDC-IDRI metadata file",
    )
    parser.add_argument(
        "-s",
        "--spacing",
        default=[0.703125, 0.703125, 1.25],
        help="target spacing for transformed images (TODO: currently does not work due to unknown reason)",
    )
    ...
    # 2. prepare data
    meta_dict = {}
    with open(args.dicom_meta_data_csv, newline="") as csvfile:
        print("open " + args.dicom_meta_data_csv)
        reader = csv.DictReader(csvfile)
        for row in reader:
            meta_dict[row["File Location"][12:]] = str(row["Series UID"])

    for data_list_key in ["calibration"]:
